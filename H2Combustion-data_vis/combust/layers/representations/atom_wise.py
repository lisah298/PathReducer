...
class ShellProvider(nn.Module):
    ...
    def forward(self,
        ...
        if atom_idx is not None:
            ...
            # sanity check
            if atom_idx.size()[0] != neighbors.size()[0] and neighbors.size(
            )[0] > atoms.size()[0]:
            ...
            distance_vector = target_nbh_pos - target_pos[:,
                                                          None, :]  # A', N, 3
            ...
            if cell is not None:  # 3, 3
                ...
                shift = torch.bmm(
                    shift_vector.view(dim[0], dim[1] * dim[2], dim[3]),
                    cell)  # B, A'*N, 3
            ...
            distance_vector = target_nbh_pos - atoms[:, :,
                                                     None, :]  # B, A, N, 3
