...
def gradient_color_change_magic(x, y, z=None):
    if z is None:
        ...
        lc = LineCollection(segments, array=x_arr, cmap='viridis', norm=plt.Normalize(
            0.0, 1.0), alpha=0.8, linewidth=2)
        ...
        segments = np.concatenate([points[:-1], points[1:]], axis=1)
        lc = Line3DCollection(
            segments, array=x_arr, cmap='viridis', norm=plt.Normalize(0.0, 1.0), alpha=0.8)
...
def colored_line_plot(x, y, z=None, x2=None, y2=None, z2=None, imgname=None, same_axis=False, lengths=None,
    ...
    if points_to_circle is not None:
        for i in points_to_circle:
            ax0.scatter(x[i], y[i], edgecolors='k',
                        facecolors='none', s=50, zorder=5)
            if z is not None:
                ax1.scatter(x[i], y[i], z[i], edgecolors='k',
                            facecolors='none', s=50, zorder=5)
...
def colored_line_plot_projected_data(x, y, new_data_x=None, new_data_y=None, z=None, new_data_z=None, x2=None, y2=None,
    ...
    if lengths is None:
        ...
        if same_axis:
            ...
            ax0.set_xlim([x_together.min()-0.1*xrange_,
                         x_together.max()+0.1*xrange_])
            ax0.set_ylim([y_together.min()-0.1*yrange_,
                         y_together.max()+0.1*yrange_])
    ...
    if new_data_x is not None and new_data_y is not None:
        ...
        ax0.set_xlim([x_i_new.min() - 0.1 * xrange_,
                     x_i_new.max() + 0.1 * xrange_])
        ax0.set_ylim([y_i_new.min() - 0.1 * yrange_,
                     y_i_new.max() + 0.1 * yrange_])
        # ax0.set_xlim(-40, 100)
        # ax0.set_ylim(-20, 20)
    ...
    if lengths is None or sum(lengths) == 0:
        ...
        if same_axis:
            ...
            ax1.set_xlim([x_together.min()-0.1*xrange_,
                         x_together.max()+0.1*xrange_])
            ax1.set_ylim([y_together.min()-0.1*yrange_,
                         y_together.max()+0.1*yrange_])
            ax1.set_zlim([z_together.min()-0.1*zrange_,
                         z_together.max()+0.1*zrange_])
    ...
    if new_data_z is not None:
        tck_new, u_new = interpolate.splprep(
            [new_data_x, new_data_y, new_data_z], k=1, s=0.0)
        x_i_new, y_i_new, z_i_new = interpolate.splev(
            np.linspace(0, 1, 1000), tck_new)
        ...
        for i in points_to_circle_new_data:
            ...
            if new_data_z is not None:
                ...
                ax1.scatter(new_data_x[i], new_data_y[i], new_data_z[i],
                            zorder=5, edgecolors='k', marker="$\u263c$", s=200)
...
def colored_line_and_scatter_plot(x, y=None, y1=None, x2=None, y2=None, y12=None, imgname=None, same_axis=False, lengths=None, new_data=None,
    ...
    if y is None and y1 is None:
        ...
        if lengths is None:
            ...
            ax0.scatter(x, y, s=50, c=time, cmap='viridis',
                        zorder=10, alpha=0.6)
            ...
            if x2 is not None and y2 is not None:
                ...
                ax0.scatter(x2, y2, s=50, c=time,
                            cmap='viridis', zorder=10, alpha=0.6)
            ...
            for i in range(len(lengths)):
                if i == 0:
                    ...
                    ax0.scatter(xseg, yseg, s=50, c=time,
                                cmap='viridis', zorder=10)
                    if mark_first_last:
                        ax0.scatter(xseg.iloc[[0]], yseg.iloc[[
                                    0]], edgecolors='k', linewidth=1.5, facecolors='none', s=200, zorder=10)
                        ax0.scatter(xseg.iloc[[-1]], yseg.iloc[[-1]], edgecolors='k',
                                    linewidth=1.5, facecolors='none', s=200, zorder=10)
            ...
            if same_axis:
                ...
                xrange_ = x.max() - x.min()
                yrange_ = y.max() - y.min()
                ax0.set_xlim([x.min() - 0.1 * xrange_,
                             x.max() + 0.1 * xrange_])
                ax0.set_ylim([y.min() - 0.1 * yrange_,
                             y.max() + 0.1 * yrange_])

        # Adding new_data, if it exists
        if new_data is not None:
            tck_new, u_new = interpolate.splprep(
                [new_data[0], new_data[1]], k=1, s=0.0)

            x_i_new, y_i_new = interpolate.splev(
                np.linspace(0, 1, 1000), tck_new)
            ...
            lc_new = LineCollection(segments_new, array=z_new, cmap='viridis', norm=plt.Normalize(0.0, 1.0),
                                    linestyle="solid", linewidth=2.5)
            lca_new = LineCollection(
                segments_new, color='black', linestyle="solid", linewidth=6.0)
            ax0.add_collection(lca_new)
            ax0.add_collection(lc_new)
            time = list(range(len(new_data[0])))
            ax0.scatter(new_data[0], new_data[1], s=5,
                        c=time, cmap='viridis', zorder=11)
            # BUG
    ...
    if y1 is not None:
        ...
        if lengths is None or sum(lengths) == 0:
            ...
            lc = Line3DCollection(segments, array=z, cmap='viridis', norm=plt.Normalize(0.0, 1.0), alpha=0.8,
                                  linewidth=2)
            ax1.add_collection(lc)
            ax1.scatter(x, y, y1, s=50, c=list(
                range(len(x))), cmap='viridis', alpha=0.6)

            if x2 is not None and y2 is not None and y12 is not None:
                tck, u = interpolate.splprep([x2, y2, y12], k=1, s=0.0)
                x_i2, y_i2, z_i2 = interpolate.splev(
                    np.linspace(0, 1, 1000), tck)
                ...
                ax1.add_collection(lc2)
                ax1.scatter(x2, y2, y12, s=50, c=list(
                    range(len(x2))), cmap='viridis', zorder=1)
            ...
            for i in range(len(lengths)):
                if i == 0:
                    ...
                    tck, u = interpolate.splprep(
                        [xseg, yseg, zseg], k=1, s=0.0)
                    x_i, y_i, z_i = interpolate.splev(
                        np.linspace(0, 1, 1000), tck)
                    ...
                    lc = Line3DCollection(
                        segments, array=z, cmap='viridis', norm=plt.Normalize(0.0, 1.0), alpha=0.8)
                    ax1.add_collection(lc)
                    ax1.scatter(xseg, yseg, zseg, s=50, c=list(
                        range(len(xseg))), cmap='viridis')
                    if mark_first_last:
                        ax1.scatter(xseg.iloc[[0]], yseg.iloc[[0]], zseg.iloc[[
                                    0]], edgecolors='k', linewidth=1.5, facecolors='none', s=200, zorder=10)
                        ax1.scatter(xseg.iloc[[-1]], yseg.iloc[[-1]], zseg.iloc[[-1]],
                                    edgecolors='k', linewidth=1.5, facecolors='none', s=200, zorder=10)
        ...
        if new_data is not None:
            tck_new, u_new = interpolate.splprep(
                [new_data[0], new_data[1], new_data[2]], k=1, s=0.0)
            x_i_new, y_i_new, z_i_new = interpolate.splev(
                np.linspace(0, 1, 1000), tck_new)
            ...
            z_new = np.linspace(0.0, 1.0, x_i_new.shape[0])
            points_new = np.array(
                [x_i_new, y_i_new, z_i_new]).T.reshape(-1, 1, 3)
            segments_new = np.concatenate(
                [points_new[:-1], points_new[1:]], axis=1)
            lc_new = Line3DCollection(segments_new, array=z_new, cmap='viridis', norm=plt.Normalize(0.0, 1.0),
                                      linestyle="solid", linewidth=2.5)
            ...
            ax1.add_collection(lca_new)
            ax1.add_collection(lc_new)
            ax1.scatter(new_data[0], new_data[1], new_data[2], s=5, c=list(
                range(len(new_data[0]))), cmap='viridis')
            ...
            ax1.set_xlim([x_i_new.min() - 0.1 * xrange_,
                         x_i_new.max() + 0.1 * xrange_])
            ax1.set_ylim([y_i_new.min() - 0.1 * yrange_,
                         y_i_new.max() + 0.1 * yrange_])
            ax1.set_zlim([z_i_new.min() - 0.1 * zrange_,
                         z_i_new.max() + 0.1 * zrange_])

    if points_to_circle is not None:
        for i in points_to_circle:
            ax0.scatter(x[i], y[i], edgecolors='k', linewidth=1.5,
                        facecolors='none', s=200, zorder=10)
            ax1.scatter(x[i], y[i], y1[i], edgecolors='k',
                        linewidth=1.5, facecolors='none', s=200, zorder=10)
...
def colored_scatter_plot(x, y, z, directory=None, points_to_circle=None, imgname=None):
    ...
    if points_to_circle is not None:
        ...
        for i in points_to_circle:
            ax0.scatter(x[i], y[i], edgecolors='k',
                        facecolors='none', s=400, linewidth=2, zorder=5)
            ax1.scatter(x[i], y[i], z[i], edgecolors='k',
                        facecolors='none', s=400, linewidth=2, zorder=5)

        filename = "%s_scatter.eps" % imgname

        fig0.savefig(directory + "/" + "2D_" + filename,
                     format='eps', bbox_inches='tight')
        fig1.savefig(directory + "/" + "3D_" + filename,
                     format='eps', bbox_inches='tight', pad_inches=0.5)
...
def plot_irc(path, imgname=None, output_directory=None, points_to_circle=None):
    ...
    ax.set_xlabel(
        "Intrinsic Reaction Coordinate (Bohr$\sqrt{amu}$)", fontsize=16)
    ...
    if points_to_circle is not None:
        for i in points_to_circle:
            ax.scatter(x[i], y[i], s=400, facecolors='none',
...
def plot_irc_for_gif(path, gif_points_to_circle, imgname=None, output_directory=None):
    ...
    for i in gif_points_to_circle:
        ...
        plt.savefig(os.path.join(output_directory, imgname +
                    "_scatterline_%s.pdf" % i), format='pdf')
...
def plot_pcs_for_gif_2D(pca_matrix, gif_points_to_circle, imgname=None, output_directory=None):
    ...
    for i in gif_points_to_circle:
        ...
        lc = gradient_color_change_magic(x, y)
        ax.add_collection(lc)
        ax.scatter(x, y, s=100, c=list(range(len(x))),
                   cmap='viridis', edgecolors='k', zorder=2)
        ...
        ax.scatter(x[i], y[i], s=400, facecolors='none',
                   edgecolors="black", linewidth=2, zorder=-1)
...
def plot_pcs_for_gif_3D(pca_matrix, gif_points_to_circle, imgname=None, output_directory=None):
    ...
    for i in gif_points_to_circle:
        ...
        lc = gradient_color_change_magic(x, y, z)
        ax.add_collection(lc)
        ax.scatter(x, y, z, s=100, c=list(range(len(x))),
                   cmap='viridis', edgecolors='k', zorder=2)
        ...
        ax.scatter(x[i], y[i], z[i], s=400, facecolors='none',
                   edgecolors="black", linewidth=2, zorder=-1)
        # Set viewing "distance" to plot, so axes aren't cut off in final images
        ax.dist = 9
        plt.savefig(os.path.join(output_directory, imgname +
                    "_3PCs_scatterline_%s.pdf" % i), format='pdf')
...
def plot_pcs_for_gif_3D_rotating(pca_matrix, gif_points_to_circle, imgname=None, output_directory=None):
    ...
    for i in gif_points_to_circle:
        ...
        ax.dist = 11
        plt.savefig(os.path.join(output_directory, imgname +
                    "_3PCs_scatterline_rotating_%s.pdf" % i), format='pdf')
...
def plot_prop_of_var(values, system_name=None, output_directory=None):
    ...
    if system_name is None or output_directory is None:
        ...
        fig.savefig(os.path.join(output_directory,
                    '%s_proportion_of_variance.png' % system_name), dpi=600)
